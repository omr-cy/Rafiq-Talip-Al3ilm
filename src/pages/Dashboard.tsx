  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1 px-2 md:px-0">
        <h1 className="font-serif text-2xl md:text-3xl font-bold text-olive-900">
      ...
      <section className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-olive-800 via-olive-900 to-olive-950 text-paper p-5 md:p-12 shadow-2xl shadow-olive-900/30 border border-olive-700/50 min-h-[14rem] md:min-h-[24rem] flex flex-col justify-center">
          ...
          {ayah ? (
            <>
              <motion.div
                ...
                className="w-full flex items-center justify-center min-h-[8rem] md:min-h-[14rem] px-2 py-2"
              >
                <p
                  className={cn(
                    "font-serif text-paper max-w-4xl mx-auto drop-shadow-md py-2 tracking-wider",
                    ayah.text.length > 150 ? "text-lg md:text-3xl lg:text-4xl" : 
                    ayah.text.length > 80 ? "text-xl md:text-4xl lg:text-5xl" : 
              ...
              <motion.div 
                ...
                className="flex flex-wrap items-center justify-center gap-3 md:gap-5 text-olive-100"
              >
                <span className="text-sm md:text-base font-medium bg-olive-950/50 px-6 py-2.5 rounded-full border border-olive-700/50 shadow-sm backdrop-blur-sm">
                  سورة {ayah.surah} - الآية{" "}
                  {ayah.ayahNumber.toLocaleString("ar-EG")}
                </span>
                <button
                  onClick={() => setShowTafsir(!showTafsir)}
                  className="text-sm md:text-base font-bold bg-sand text-olive-950 hover:bg-sand-light px-8 py-2.5 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 active:scale-95"
              ...
              <AnimatePresence>
                {showTafsir && (
                  ...
                  >
                    <div className="mt-8 p-6 md:p-8 bg-olive-950/50 backdrop-blur-xl rounded-3xl border border-olive-700/60 shadow-inner relative">
...
const StatCard = memo(function StatCard({
  ...
  return (
    <motion.div
      ...
      className="optimize-gpu bg-card rounded-[2rem] p-6 md:p-8 border border-olive-100 shadow-lg shadow-olive-900/5 flex flex-col gap-5 relative overflow-hidden group transition-all duration-300 hover:shadow-xl hover:shadow-olive-900/10 hover:border-olive-200"
    >
      ...
      <div className="relative z-10 flex items-center gap-4">
        ...
        <div>
          <h3 className="text-olive-700 font-bold text-sm md:text-base mb-1 group-hover:text-olive-900 transition-colors">{title}</h3>
      ...
      <div className="relative z-10 mt-1">
         <span className="text-xs md:text-sm font-medium text-olive-600 bg-olive-50/80 px-4 py-1.5 rounded-full border border-olive-100/50 inline-block shadow-sm">{subtitle}</span>
      </div>
    </motion.div>
  );
});
